...
const avatar = require(path.join(appPath, "util/avatar.js"))
    ...
    $('#user-info .avatar').on('click', function () {
      remote.dialog.showOpenDialog({
        properties: ['openFile', 'multiSelections'], filters: [
          { name: 'Image', extensions: ['jpg', 'png', 'gif', 'jpg'] },
        ]
      })
        .then(result => {
          if (result.canceled) {
          ...
          localStorage.setItem("avatar", avatar.base64Encode(result.filePaths[0]));
          location.reload();
        }).catch(err => {
    ...
    achievements.makeList(self.config, (percent) => {
      ...
      $("#game-list .game-box").contextmenu(function (e) {
        ...
        menu.append(new MenuItem({
          icon: nativeImage.createFromPath(path.join(appPath, "resources/img/cross.png")), label: $("#game-list").attr("data-contextMenu0"), click() {
        ...
        if (!self.data("system")) //Steam only
        {
          if (app.config.notification_advanced.iconPrefetch) {
            menu.append(new MenuItem({
              icon: nativeImage.createFromPath(path.join(appPath, "resources/img/image.png")), label: $("#game-list").attr("data-contextMenu1"), async click() {
          ...
          menu.append(new MenuItem({
            icon: nativeImage.createFromPath(path.join(appPath, "resources/img/file-text.png")), label: "Generate achievements.json for Goldberg Emu", async click() {
              self.css("pointer-events", "none");
              try {
